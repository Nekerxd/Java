 ...
 * @author Nuvie
 */
@WebServlet(name = "MetaExcluir", urlPatterns = {"/MetaExcluir"})
public class MetaExcluir extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            if (dao.excluir(idMeta))
                mensagem = "Meta excluída com sucesso!";
            else
                mensagem = "Problemas ao excluir Meta!";
            request.setAttribute("mensagem", mensagem);
            response.sendRedirect("MetaListar");
        } catch (Exception ex) {
            System.out.println("Problemas no Servlet ao excluir Meta! Erro: "+ex.getMessage());
            ex.printStackTrace();
        }
